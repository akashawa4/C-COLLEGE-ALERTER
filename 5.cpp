...
public:
    ...
    void electionSystem() {
        cout << "\n--- Election System ---\n";
        cout << "Candidates:\n";
        for (size_t i = 0; i < electionCandidates.size(); ++i) {  // Change int to size_t
        ...
        cin >> voteChoice;
        if (voteChoice > 0 && voteChoice <= static_cast<int>(electionCandidates.size())) {  // Cast size_t to int for comparison
    ...
    // Method for Admin to register a new user
    void registerNewUser() {
        ...
        cout << "Enter new user's role (Student, Teacher, Non-Teaching Staff): ";
        cin >> role;

        // Check role validity
        if (role == "Student" || role == "Teacher" || role == "Non-Teaching Staff") {
    ...
    void showRoleOptions(const string& role) {
        int action;
        if (role == "Student") {
            cout << "\nStudent Options:\n";
            cout << "1. Vote in Election\n2. Submit Complaint\n3. Exit\n";
            cout << "Enter your choice: ";
            cin >> action;
            switch (action) {
                case 1: electionSystem(); break;
                case 2: complaintSystem(); break;
                case 3: break;
    ...
    // Initialize the system with a default admin user (akashaw5)
    void initSystem() {
        registerUser("akashaw5", "akashaw5@gmail.com", "0147", "Admin");
        initElection();
    }

    // Add this method to retrieve user role by email
    string getUserRoleByEmail(const string& email) {
        for (const auto& userEntry : users) {
            if (userEntry.second.email == email) {
                return userEntry.second.role;
            }
        }
        return "";  // Return an empty string if the email doesn't match any user
    }
};
...
int main() {
    ...
    // Initialize the system with the default admin user
    system.initSystem();
    ...
    if (system.authenticateUser(email, password)) {
        string role = system.getUserRoleByEmail(email);  // Get role using the new method
        cout << "Welcome, " << role << "!\n";
        system.showRoleOptions(role);  // Show options based on role
    } else {
